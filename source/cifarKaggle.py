...
class CifarKaggle():
	...
	Một thể hiện của lớp này được tạo ra 
	bên trong phương thức merge_with_cifarkaggle của lớp OCR
	để trộn dữ liệu CIFAR-10 dataset on Kaggle với dữ liệu có 
	văn bản của tập 74K Chars dataset
	"""

	def __init__(self, config):
		"""
		Lấy các tham số từ 1 file config.py (kiểu json)
		"""
	...
	def _load_config(self, filename):
		"""
		Đọc file config.py và trả về dictionary cho tất cả các tham sô
		"""
		return eval(open(filename).read())

	def load(self):
		"""
		Load tập cifar vào trong python dictionary
		"""
		...
		if self.from_pickle:
			try:
				...
				print 'You have to provide a (*.pickle) to load data from'
				sys.exit(0)
		else:
	...
	def plot_some(self):
		"""
		vẽ 100 ảnh kèm nhãn một cách ngẫu nhiên lấy từ tập dữ liệu đã load
		"""
